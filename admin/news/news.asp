...
sql = "Select roomid from news where roomid="& id &" and news_id="& newid
...
sql="select * from news where roomid="& id &" order by news_id DESC"
set rs=server.createobject("adodb.recordset")
  ...
  <tr> 
    <td width="636" bgcolor="#FFFFFF" valign="top" align="center"> 
      ...
      <table width="623" border="0" cellspacing="1" cellpadding="4" bgcolor="#293863">
        <tr bgcolor="#eeeeee"> 
          <td colspan="4" class="diaryhead">新闻管理</td>
        </tr>
        <tr bgcolor="#E8E8E8" align="center"> 
          <td class="chinese" width="31">编号</td>
          <td class="chinese" width="45">分类</td>
          <td class="chinese" width="394">标题&amp;发表时间</td>
          <td class="chinese" width="116">操作</td>
        </tr>
<%if not rs.eof then
rs.movefirst
rs.pagesize=8
...
do while not rs.eof and i<newsperpage%>
        <tr> 
          ...
          <td bgcolor="#FFFFFF" class="chinese" align="center">
			...
			编辑</a> 
            <a href="news.asp?id=<%=rs("news_id")%>&action=delnews#delnews">删除</a> <a href="shownews.asp?news_id=<%=rs("news_id")%>" target="_blank">观看</a></td>
        </tr>
<%i=i+1
rs.movenext
loop
...
if rs.eof and rs.bof then%>
        <tr align="center"> 
          <td bgcolor="#FFFFFF" colspan="4" class="chinese">当前没有新闻！</td>
        </tr>
<%end if
end if%>
      </table>
