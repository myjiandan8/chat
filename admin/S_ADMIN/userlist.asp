...
sql="select * from admin where roomid="& rmport
...
<table width="597" border="1" bordercolordark=#9CC7EF bordercolorlight=#145AA0 cellspacing="0" cellpadding="4" style="border-collapse: collapse" bordercolor="#111111">
  <tr> 
    <form method="post" action="memberList.asp" name="form1">
      <td colspan="3" bgcolor="#4296E7" height="28" width="737"> 
  ...
  <%
		...
		rsID=rs("id")
%>
  <tr title="点击查看具体信息" style="cursor:hand" onClick="Javascript:window.location='useradd.asp?menu=edit&name=<%=rs("name")%>'"> 
    <td width="100">
      <div align="center"><%=rs("name")%>　</div>
    </td>
    <td width="150">
      <div align="center"><%=rs("cname")%>　</div>
    </td>
    <td width="249">
      <p align="center">　<%=rs("txt")%></td>
  </tr>
  <%
		i=i+1
