...
<!--#include file="lib/adminpic_body.asp"-->
<title>视频聊天 语音聊天 聊湾 蓝天语音-图片管理</title>
<%
dim founderr,errmsg
founderr=false
errmsg=""
...
if founderr then
  ...
  conn.execute(sql)
  closedatabase
  response.redirect "admin_pic.asp?action=cscat"
end if
...
if csurl="" then
  founderr=true
  errmsg=errmsg+"<br>"+"<li>你必须填写地址！"
else
  rs("cs_url")=csurl
end if
if cspic="" then
  founderr=true
  errmsg=errmsg+"<br>"+"<li>你必须填写的图片地址！"
else
  rs("cs_pic")=cspic
end if
if csdesc="" then
  founderr=true
  errmsg=errmsg+"<br>"+"<li>你必须填写的简要说明！"
else
  rs("cs_desc")=csdesc
end if
...
if founderr then
  ...
  response.cookies("0")=""
  response.cookies("1")=""
  response.redirect "admin_pic.asp?action=sites"
end if
...
openpic
sql="select * from pic where cs_id="&cint(request.form("id"))
set rs=server.createobject("adodb.recordset")
rs.open sql,conn,1,3
cscatid=cint(request.form("cscatid"))
csname=trim(replace(request.form("name"),"'",""))
