...
<!--#include file="lib/adminmodel_body.asp"-->
<title>视频聊天 语音聊天 聊湾 蓝天语音-模板管理</title>
<%
dim founderr,errmsg
founderr=false
errmsg=""
...
if founderr then
  ...
  conn.execute(sql)
  closedatabase
  response.redirect "admin_model.asp?action=cscat"
end if
...
rs.open sql,conn,1,3
rs.addnew
dim cscatid,csname,csurl,cspic,csdesc,csisbest,showurl
cscatid=cint(request.form("cscatid"))
csname=trim(replace(request.form("name"),"'",""))
showurl=trim(replace(request.form("showurl"),"'",""))
...
if csname="" then
  founderr=true
  errmsg=errmsg+"<br>"+"<li>你必须填写模板名称！"
else
  rs("cs_name")=csname
end if
if cscatid<1 then
  founderr=true
  errmsg=errmsg+"<br>"+"<li>你必须选择模板所属分类！"
else
  rs("cscat_id")=cscatid
end if
if csurl="" then
  founderr=true
  errmsg=errmsg+"<br>"+"<li>你必须填写模板地址！"
else
  rs("showurl")=showurl
  rs("cs_url")=csurl
end if
if cspic="" then
  founderr=true
  errmsg=errmsg+"<br>"+"<li>你必须填写模板的图片地址！"
else
  rs("cs_pic")=cspic
end if
if csdesc="" then
  founderr=true
  errmsg=errmsg+"<br>"+"<li>你必须填写模板的简要说明！"
else
  rs("cs_desc")=csdesc
end if
...
if founderr then
  ...
  rs.close
  set rs=nothing
  response.redirect "admin_model.asp?action=sites"
end if
