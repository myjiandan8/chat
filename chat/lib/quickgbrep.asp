...
<table width="98%" border="0" align="center" cellpadding="0" cellspacing="0">
        <tr> 
          ...
          <td background="images/tablebg.gif">::::: <B>回复留言</B> :::::<a class=buttonview 
  ...
  <table width="90%" border="0" cellspacing="1" align="center">
    <form name="addgb" method="post" action="<%=MM_editAction%>">
	<tr> 
      <td> 
        <P class=columnright><br>
          姓名: 
          <input type="text" name="rep_poster" class="input2" maxlength="50">
      </td>
    </tr>
    <tr> 
      <td> 
        <P class=columnright>
          <input type="button" name="Bold2" value="Bold" onClick=Cbold()>
          <input type="button" name="Italic2" value="Italic" onClick=Citalic()>
          <input type="button" name="Underline2" value="Under" onClick=Cunder()>
          <input type="button" name="Center2" value="Center" onClick=Ccenter()>
          <input type="button" name="Url2" value="Url" onClick=Curl()>
          <input type="button" name="Image2" value="Image" onClick=Cimage()>
          <input type="button" name="Fly2" value="Fly" onClick=Cfly()>
          字体 
          <select onChange="if(this.options[this.selectedIndex].value!=''){showfont(this.options[this.selectedIndex].value);this.options[0].selected=true;}else {this.selectedIndex=0;}" name=select2>
          ...
          &nbsp;&nbsp;&nbsp;&nbsp; 字体大小 
          <select name="select2" onChange="if(this.options[this.selectedIndex].value!=''){showsize(this.options[this.selectedIndex].value);this.options[0].selected=true;}else {this.selectedIndex=0;}">
          ...
          &nbsp;&nbsp;&nbsp; 颜色 
          <select onChange="if(this.options[this.selectedIndex].value!=''){showcolor(this.options[this.selectedIndex].value);this.options[0].selected=true;}else {this.selectedIndex=0;}" name=select2>
    ...
    <tr> 
      <td><P class=columnright> 
          <textarea name="gb_content" rows="12" class="textfield" cols="80"></textarea>
      </td>
    </tr>
    <tr> 
      <td align="center"> 
        <input type="submit" name="Submit" value="提交回复" class="input2" onClick="YY_checkform('addgb','rep_poster','#q','0','回复者姓名必须填写','gb_content','1','1','回复内容不可为空');return document.MM_returnValue">
        <input type="reset" name="reset" value="清空重写" class="input2">
        <input type="hidden" name="gb_id" value="<%=request.querystring("gb_id")%>">    <input type="hidden" name="MM_insert" value="true">
      </td>
    </tr>
</form>
  </table><br>
<table width="100%" border="0" cellspacing="1" bgcolor="#f9f9f9">
  <tr> 
    <td> 
...
rsrep.open sql,conn,1,1%>
        <tr> 
          <td valign=center colspan=2> 
            <strong><%=rs("gb_poster")%></strong> <%if rs("gb_posterfrom")<>"" then%>(<%=rs("gb_posterfrom")%>)<%end if%> 
			  <%if rs("gb_postersex")=0 then%>
              <img src="images/b.gif" border="0">
              <%else%>
			  <img src="images/b.gif" border="0">
			  <%end if%>
			  <%if rs("gb_posteremail")<>"" then%>
              <a href="<%=rs("gb_posteremail")%>" target="_blank"><img src="images/email.gif" alt="<%=rs("gb_posteremail")%>" border="0"></a> 
              <%end if%>
              ...
              <a href="http://search.tencent.com/cgi-bin/friend/user_show_info?ln=<%=rs("gb_posterqq")%>"><img src="images/profile.gif" alt="qq:<%=rs("gb_posterqq")%>" border="0"></a> 
              <%end if%>
           <BR><%=ubb2html(formatStr(autourl(rs("gb_content"))), true, true)%> 
            <p class=entry3date><%=rs("gb_date")%> &nbsp;(<%=rs("gb_id")%>) &nbsp;<a href="New_gb.asp?action=rep&gb_id=<%=rs("gb_id")%>"><img src="images/gb_comment.gif" alt="回复" border="0" align="middle"></a></p>
            <%
