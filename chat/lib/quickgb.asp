...
<table width="98%" border="0" align="center" cellpadding="0" cellspacing="0" align="center">
    ...
    <TBODY> <form name="addgb" method="POST" action="guestbook.asp?<%=MM_editAction%>">
    <tr> 
      <td width="6%"> 
        <P class=columnright><br>
          姓名: 
      </td>
      <td width="94%"> 
        <P class=columnright> <br>
          <input type="text" name="gb_poster" class="input" maxlength="50">
          <font color="#FF6600"> *</font><font color="#FFD300"> </font>最多字符50 
      </td>
    </tr>
    <tr> 
      <td width="6%"> 
        <P class=columnright>性别: 
      </td>
      <td width="94%">
        <P class=columnright> 
          <input type="radio" name="gb_postersex" value="0" checked>
          男 
          <input type="radio" name="gb_postersex" value="1">
          女&nbsp; 
      </td>
    </tr>
    <tr> 
      <td width="6%"> 
        <P class=columnright>QQ: &nbsp; 
      </td>
      <td width="94%">
        <P class=columnright> 
          <input type="text" name="gb_posterqq" class="input" maxlength="20">
          最多字符20 
      </td>
    </tr>
    <tr> 
      <td width="6%"> 
        <P class=columnright>Email: 
      </td>
      <td width="94%">
        <P class=columnright> 
          <input type="text" name="gb_posteremail" class="input" maxlength="30">
          最多字符30 
      </td>
    </tr>
    <tr> 
      <td width="6%"> 
        <P class=columnright>主页: 
      </td>
      <td width="94%">
        <P class=columnright> 
          <input type="text" name="gb_posterhome" class="input" maxlength="50">
          最多字符50 
      </td>
    </tr>
    <tr> 
      <td width="6%"> 
        <P class=columnright>来自: 
      </td>
      <td width="94%">
        <P class=columnright> 
          <input type="text" name="gb_posterfrom" class="input" maxlength="50">
		  <input type="hidden" name="gb_posterip" class="input" maxlength="50" value="<%response.write Request.ServerVariables("REMOTE_ADDR")%>">
          最多字符50 
      </td>
    </tr>
    <tr> 
      <td colspan="2"> <BR>
          <input type="button" name="Bold" value="Bold" onClick=Cbold()  class=inputs>
          <input type="button" name="Italic" value="Italic" onClick=Citalic() class=inputs>
          <input type="button" name="Underline" value="Under" onClick=Cunder() class=inputs>
          <input type="button" name="Center" value="Center" onClick=Ccenter() class=inputs>
          <input type="button" name="Url" value="Url" onClick=Curl() class=inputs>
          <input type="button" name="Image" value="Image" onClick=Cimage() class=inputs>
          <input type="button" name="Fly" value="Fly" onClick=Cfly() class=inputs><br><BR>
          字体 
          <select onChange="if(this.options[this.selectedIndex].value!=''){showfont(this.options[this.selectedIndex].value);this.options[0].selected=true;}else {this.selectedIndex=0;}" name=select>
          ...
          &nbsp;&nbsp;&nbsp;&nbsp; 字体大小 
          <select name="select" onChange="if(this.options[this.selectedIndex].value!=''){showsize(this.options[this.selectedIndex].value);this.options[0].selected=true;}else {this.selectedIndex=0;}">
          ...
          &nbsp;&nbsp;&nbsp; 颜色 
          <select onChange="if(this.options[this.selectedIndex].value!=''){showcolor(this.options[this.selectedIndex].value);this.options[0].selected=true;}else {this.selectedIndex=0;}" name=select>
    ...
    <tr> 
      <td colspan="2"> 
          <textarea name="gb_content" rows="12" cols="80" class="textarea"></textarea>
      </td>
    </tr>
    <tr> 
      <td height="30" colspan="2"> 
        <div align="center"> 
          <input type="submit" name="Submit" value="提交留言" class="input" "YY_checkform('addgb','gb_poster','#q','0','回复者姓名必须填写','gb_content','1','1','留言内容不可为空');return document.MM_returnValue onClick="YY_checkform('addgb','gb_poster','#q','0','姓名必须填写！','gb_content','1','1','留言内容必须填写');return document.MM_returnValue"">
          <input type="reset" name="reset" value="清空重写" class="input">
        </div>
      </td>
    </tr>
