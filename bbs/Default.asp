...
<%		
		...
		set rs2=chatconn.execute("select url,name from link where roomid="& roomid &" and lx=0")
		dim i
		i=rs2.recordcount
...
<%		set rs2=chatconn.execute("select name,url,logo from link where roomid="& roomid &" and lx=1")
		i=rs2.recordcount
		if rs2.eof and rs2.bof then
		...
		do while not rs2.eof
%>
<a title="<%=rs2("name")%>" target="_blank" href="<%=rs2("url")%>">
<img border="0" src="<%=rs2("logo")%>" width="88" height="31"></a><%rs2.movenext
		  loop
		  end if
		  rs2.close
		  %>  
...
if ListStyle=0 and roomid=0 then
